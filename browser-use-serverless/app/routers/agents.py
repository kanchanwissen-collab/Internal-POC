...
# Feature flags / settings (env-driven)
AGENT_NAME = os.getenv("AGENT_NAME", "browser agent")

# publish to Pub/Sub in addition to Streams? (default ON)
PUBLISH_TO_PUBSUB = os.getenv("LOG_PUBSUB", "1") == "1"

# publish JSON payload on Pub/Sub? (default OFF -> plain text)
PUBSUB_JSON = os.getenv("LOG_PUBSUB_JSON", "0") == "1"

# cap Stream length (approx. trim). "0" disables trimming
STREAM_MAXLEN_ENV = os.getenv("REDIS_STREAM_MAXLEN", "0")
STREAM_MAXLEN = int(STREAM_MAXLEN_ENV) if STREAM_MAXLEN_ENV.isdigit() else 0
...
class AgentCreateRequest(BaseModel):
    ...
    request_id: str = Field(..., description="Request ID for tracking and Redis log streaming")
...
async def create_agent(body: AgentCreateRequest):
    """
    Start an agent and stream logs to Redis (Streams + Pub/Sub).
    This endpoint blocks until the agent finishes.
    """
    ...
    try:
        ...
        print(f"🔄 [Agent] Starting agent for request_id: {request_id}")
        print(f"📡 [Agent] Redis logging to stream/channel: {stream_key}")
        ...
        class RedisHandler(logging.Handler):
            ...
            def emit(self, record: logging.LogRecord):
                try:
                    ...
                    # 1) Write to Stream (history/backfill)
                    if STREAM_MAXLEN > 0:
                        r.xadd(self.stream_key, {"msg": formatted}, maxlen=STREAM_MAXLEN, approximate=True)
                    else:
                        r.xadd(self.stream_key, {"msg": formatted})

                    # 2) Also publish live to Pub/Sub so existing FE subscriber receives it
                    if PUBLISH_TO_PUBSUB:
                        if PUBSUB_JSON:
                            ...
                            payload = formatted  # keep FE happy if it expects plain text
                        r.publish(self.stream_key, payload)
...
""",
        ...
        # --- tee stdout/stderr for key agent lines into Redis (Stream + Pub/Sub) ---
            ...
            def write(self, s: str):
                ...
                for line in s.splitlines():
                    if self.pattern.search(line):
                        ...
                        # 1) Stream (history/backfill)
                        if STREAM_MAXLEN > 0:
                            r.xadd(stream_key, {"msg": msg}, maxlen=STREAM_MAXLEN, approximate=True)
                        else:
                            r.xadd(stream_key, {"msg": msg})

                        # 2) Pub/Sub (live to FE)
                        if PUBLISH_TO_PUBSUB:
                            if PUBSUB_JSON:
                                payload = json.dumps(
                                    {
